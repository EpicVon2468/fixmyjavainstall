...
pub fn install(op: Op) -> Result<()> {
	...
	let json: String = if (jdk == JDK::Temurin || jdk == JDK::JavaSE)
		&& let MajorVersion::Number(version) = version
	{
	...
	for suffix in executable_suffixes {
		...
		let java_executable: String =
			format!("{java_home}{MAIN_SEPARATOR}bin{MAIN_SEPARATOR}java{suffix}");
